...
4. **Run the stabilization loop.** Iterate up to `--max-iterations` times, following `references/stabilize-ci.md` (which sequences the loop, holds the CI stabilization procedure, and defines the exit criteria): (1) run `om-auto-review-pr {prNumber} --autofix` verbatim (`--autofix` is explicit — this chain was instructed to fix the PR, whoever authored it), capturing its verdict and the findings it did **not** fix — that step also picks up the review feedback already posted on the PR by humans, review bots, or earlier agent passes (review bodies, conversation comments, inline diff comments) and fixes it as `INHERITED` findings, so confirm its report accounts for every one of them and treat any it left unaddressed as this loop's remaining work; (2) run the built-in CI stabilization procedure — classify each failure (real bug / test bug / flake / infra), fix the real ones with tests, push, re-check, never by weakening a test or disabling a check; (3) run `om-auto-qa-pr {prNumber}` when the diff touches a user-facing surface and `--no-ui` was not passed; (4) re-merge base if it advanced during the cycle. Exit when the review is approvable, all required checks are green, and UI verification passed or is n/a — or when `--max-iterations` is hit or a genuine blocker remains (then leave the PR labeled `blocked`/`changes-requested` and report it).
