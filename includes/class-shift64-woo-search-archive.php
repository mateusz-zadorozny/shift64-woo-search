...
class Shift64_Woo_Search_Archive implements Shift64_Woo_Search_Facet_Context {
	...
	private function execute_search( $search_query, $search_term, $strategy, $config, $per_page, $paged, $sort_res = array(), $filters = array() ) {
		...
		$result = 'mixed' === $strategy
			? $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, $or_logic, $stock_mode, $demote_factor, 0.0, $needles, 0.0, $search_query, $sanitized )
			: $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, $or_logic, $stock_mode, $demote_factor, 0.0, $needles, 0.4, $search_query, $sanitized );
		...
		if ( 'mixed' !== $strategy ) {
			...
			if ( ! $resolved && ! empty( $config['token_reduction_enabled'] ) && count( $terms ) > 1 ) {
				...
				if ( count( $reduced ) < count( $terms ) && ! empty( $reduced ) ) {
					...
					$t0        = microtime( true );
					$candidate = $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $reduced, $offset, $fetch_limit, $or_logic, $stock_mode, $demote_factor, 0.0, $search_query->term_coverage_needles( $reduced ), 0.4, $search_query, implode( ' ', $reduced ) );
			...
			if ( ! $resolved ) {
				...
				$t0        = microtime( true );
				$candidate = $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, $or_logic, $stock_mode, $demote_factor, 0.0, $needles, 0.0, $search_query, $sanitized );
			...
			if ( ! $resolved && ! $or_logic ) {
				...
				$t0     = microtime( true );
				$result = $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, true, $stock_mode, $demote_factor, 0.0, $needles, 0.4, $search_query, $sanitized );
			...
			if ( ! $resolved ) {
				...
				$t0     = microtime( true );
				$result = $this->execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, false, $stock_mode, $demote_factor, $min_score, $needles, 0.4, $search_query, $sanitized );
	 ...
	 * @return array|false
	 */
	private function execute_pass_query( $redis, $index_name, $ft_query, $sort_res, $terms, $offset, $fetch_limit, $or_mode, $stock_mode, $demote_factor, $min_score = 0.0, $needles = array(), $min_ratio = 0.4, $search_query = null, $search_text = '' ) {
		$sort_mode = $sort_res['mode'] ?? Shift64_Woo_Search_Sort::MODE_RELEVANCE;

		if ( Shift64_Woo_Search_Sort::MODE_RELEVANCE === $sort_mode ) {
			return $this->ft_search_relevance( $redis, $index_name, $ft_query, $terms, $fetch_limit, $or_mode, $stock_mode, $demote_factor, $min_score, $needles, $min_ratio, $search_query, $search_text );
		}
	 ...
	 * @return array|false
	 */
	private function ft_search_relevance( $redis, $index_name, $ft_query, $terms, $limit, $or_mode = false, $stock_mode = 'exclude', $demote_factor = 0.3, $min_score = 0.0, $needles = array(), $min_ratio = 0.4, $search_query = null, $search_text = '' ) {
		$args = array(
			'FT.SEARCH',
			$index_name,
			$ft_query,
		...
		if ( $search_query instanceof Shift64_Woo_Search_Query ) {
			...
			// Keep direct reflection callers compatible while production calls use
			// the same ranking chain as the dropdown and Product Collection.
