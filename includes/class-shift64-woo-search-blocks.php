...
class Shift64_Woo_Search_Blocks {
	...
	/**
	 * Render the fallback search form for a parent block saved without children.
	 *
	 * Content saved before the composable children existed contains a bare
	 * `shift64-woo-search/search` block. That form is a preserved surface, so the
	 * markup builder the removed `[shift64_woo_search]` shortcode used lives on
	 * here — as the block's fallback renderer, reachable only from the block.
	 *
	 * The form remains a regular WooCommerce product search when JavaScript or Redis
	 * is unavailable. The default input class is what the fallback autocomplete
	 * script binds to; no selector configuration is involved.
	 *
	 * @param array<string, string> $attributes Block attributes.
	 * @return string
	 */
	private function render_fallback_search_markup( $attributes = array() ) {
		static $instance = 0;
	...
	/**
	 * Render the fallback modal search for a parent block saved without children.
	 *
	 * The modal counterpart of render_fallback_search_markup(): the builder the
	 * removed `[shift64_woo_search_modal]` shortcode used, kept as the preserved
	 * `shift64-woo-search/modal-search` block's childless fallback.
	 *
	 * @param array<string, string> $attributes Block attributes.
	 * @return string
	 */
	private function render_fallback_modal_markup( $attributes = array() ) {
		static $instance = 0;
	...
	/**
	 * Render the regular search block through its fallback renderer.
	 *
	 * @param array<string, mixed> $attributes Block attributes.
	 * @return string
	 */
	public function render_search_block( $attributes ) {
		$html = $this->render_fallback_search_markup( $attributes );
	...
	public function render_modal_search_block( $attributes ) {
		$html                  = $this->render_fallback_modal_markup( $attributes );
