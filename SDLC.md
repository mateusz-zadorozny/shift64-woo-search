...
### A note on the legacy `agent:*` labels

This repository also carries an older `agent:*` label set (`agent:ready`, `agent:in-progress`, `agent:pr-open`, `agent:changes-requested`, and others). The om-skills do not read or write them. They are left in place deliberately — setup never deletes labels — but the taxonomy above is the one this process runs on. Prune the `agent:*` labels when nothing else depends on them.
...
- Apply `needs-qa` to UI changes, new features, and other user-facing behavior that needs manual exercise. In this plugin that means the search dropdown, the archive/filter UI, and the admin settings screens.
...
This mirrors what CI runs (`.github/workflows/release.yml`), with one difference worth knowing: CI runs the PHPUnit suite against a matrix of PHP 8.3, 8.4, and 8.5 with a real MySQL service and the WordPress test suite installed by `bin/install-wp-tests.sh`. Locally the gate runs against whatever PHP and test database you have. A green local gate is necessary, not sufficient — the matrix can still fail on a version you do not run.
